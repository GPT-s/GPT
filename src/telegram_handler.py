...
class TelegramHandler:
    ...
    def favorites(self, update, context):
    ...
    def in_subscribe(self, update, context):
        chat_id = update.effective_chat.id
        context.bot.send_message(chat_id=chat_id, text="구독 처리가 완료되었습니다.")
        return chat_id

    def out_subscribe(self, update, context):
        chat_id = update.effective_chat.id
    ...
    def remove_favorite(self, update, context):
        message_text = update.message.text
    ...
    def on_callback_query(self, update, context):
...
def main():
    ...
    updater.dispatcher.add_handler(CommandHandler('start', telegram_handler.start))
    updater.dispatcher.add_handler(CommandHandler('help', telegram_handler.help))
    updater.dispatcher.add_handler(CommandHandler('sub', telegram_handler.in_subscribe))
    updater.dispatcher.add_handler(CommandHandler('nosub', telegram_handler.out_subscribe))
    updater.dispatcher.add_handler(CommandHandler('favorites', telegram_handler.favorites, pass_user_data=True))
    updater.dispatcher.add_handler(CommandHandler('remove', telegram_handler.remove_favorite, pass_user_data=True))
    updater.dispatcher.add_handler(MessageHandler(Filters.text & (~Filters.command), telegram_handler.message_handler))
    updater.dispatcher.add_handler(MessageHandler(Filters.text, telegram_handler.on_callback_query))
    updater.dispatcher.add_handler(CallbackQueryHandler(telegram_handler.on_callback_query))
